...
describe("nft-platform", () => {
  ...
  it("Purchases an NFT", async () => {
    ...
    const NftTokenAccount = await getAssociatedTokenAddress(mintKey.publicKey, admin.publicKey);
    ...
    const metadataAddress = await getMetadata([Buffer.from("metadata"), TOKEN_METADATA_PROGRAM_ID.toBuffer(), mintKey.publicKey.toBuffer()])
