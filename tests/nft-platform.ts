...
describe("nft-platform", () => {
  ...
  const MAX_NFTS = 8888;
  const PURCHASE_START = 1734637262; // Current time in seconds
  const PURCHASE_END = PURCHASE_START + 5 * 60; // 1 hour later
  ...
  it("Initializes random state the program", async () => {
    const index = 1;
